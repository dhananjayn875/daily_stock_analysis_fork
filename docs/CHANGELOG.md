...
- [修复] 对 `.NS`/`.BO` 印度市场代码启用 Yfinance 专属路由：管理层日线/实时查询绕过 Akshare、Baostock、Tencent；Yfinance 保留原后缀代码，Akshare/Baostock 对不支持格式显式拒绝并触发可解释降级。
- [修复] 修复 LiteLLM 模型名省略 `gemini/`/`anthropic/`/`deepseek/` 前缀时被误判为 OpenAI/Vertex AI 导致调用失败的问题，并新增 `MARKET_REVIEW_REGION=in` 印度大盘复盘（Nifty 50、Sensex 等主要指数）。
